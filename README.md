...
- **🎲 隨機探索**：城市太多選擇困難？首頁與每個城市頁底部都有「隨機探索」按鈕，在 460 個城市裡隨機跳到一個驚喜目的地。
...
目前收錄 **175 個目的地、460 個區域**，幾乎涵蓋全球每一個國家，橫跨亞洲、歐洲、非洲、美洲、大洋洲：
...
驗證資料完整性：`node scripts/validate_data.mjs`（檢查 175 個目的地、460 區的欄位結構與 GeoJSON 對應）。
