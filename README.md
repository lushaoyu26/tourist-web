...
- **🔍 全站搜尋**：導覽列搜尋鈕或按 `/`（或 Cmd/Ctrl+K）開啟指令面板，852 個城市與 175 國中英文即時模糊搜尋，鍵盤上下鍵選取、Enter 直達。
...
- **🎲 隨機探索**：城市太多選擇困難？首頁與每個城市頁底部都有「隨機探索」按鈕，在 852 個城市裡隨機跳到一個驚喜目的地。
...
目前收錄 **175 個目的地、852 個區域**，涵蓋全球每一個國家（**每國至少 2 座城市**），橫跨亞洲、歐洲、非洲、美洲、大洋洲：
...
驗證資料完整性：`node scripts/validate_data.mjs`（檢查 175 個目的地、852 區的欄位結構與 GeoJSON 對應）。
